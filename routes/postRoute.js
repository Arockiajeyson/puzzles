...
app.get("/edit", async (req, res) => {
  try {
    ...
    if (finding.wordLength == 3) {
      console.log([finding]);
      res.render("editing.jade", {
        edit: [finding],
        exnames: req.session.deleteExercise,
      });
    } else if (finding.wordLength == 4) {
      res.render("editing4.jade", {
        edit: [finding],
        exnames: req.session.deleteExercise,
      });
    } else if (finding.wordLength == 5) {
      res.render("editing5.jade", {
        edit: [finding],
        exnames: req.session.deleteExercise,
      });
    }
...
app.get("/continue", async (req, res) => {
  try {
    const find = await ThreeWordS.find({
      exerciseName: req.session.deleteExercise,
    });
    // console.log(find)
...
app.post("/resultChecking", async (req, res) => {
  ...
  req.session.save();
  const find = await Result.find({
    email: req.session.emailId,
    exerciseName: req.session.resultCheckingEx,
  });
  if (find.length == 0) {
    return res.json("no");
  } else {
...
app.post("/exerciseNameStoring", async (req, res) => {
  try {
    ...
    if (name != "") {
      ...
      req.session.save();
      // word=myData
      const find = await SchemaExercise.findOne({
        exerciseName: req.body.input,
        email: req.session.emailId,
      });
      // console.log(req.body)
      if (!find) {
        const create = await SchemaExercise.create({
          exerciseName: req.body.input,
          email: req.session.emailId,
        });
        create.save();
...
app.delete("/deleteex", async (req, res) => {
  try {
    ...
    req.session.save();
    const delelting = await ThreeWordS.deleteOne({
      _id: req.session.deleteExId,
    });
    const find = await ThreeWordS.find({
      email: req.session.emailId,
      exerciseName: req.session.deleteExercise,
...
app.post("/editThree", upload.array("audioEdit", 3), async (req, res) => {
  try {
    ...
    if (
      findingEx.Word1 != req.body.Word1 ||
      findingEx.Word2 != req.body.Word2 ||
      findingEx.Word3 != req.body.Word3
    ) {
      // console.log('inside')
      if (
        findingEx.Word1 != Word1 &&
        findingEx.Word2 != Word2 &&
        findingEx.Word3 != Word3
      ) {
...
app.post("/editFour", upload.array("audioEdit", 4), async (req, res) => {
  try {
    ...
    if (
      ...
      findingEx.Word2 != req.body.Word2 ||
      findingEx.Word3 != req.body.Word3 ||
      findingEx.Word4 != req.body.Word4
    ) {
      // console.log('inside')
...
app.post("/editFive", upload.array("audioEdit", 5), async (req, res) => {
  try {
    ...
    if (
      ...
      findingEx.Word3 != req.body.Word3 ||
      findingEx.Word4 != req.body.Word4 ||
      findingEx.Word5 != req.body.Word5
    ) {
      // console.log('inside')
      if (findingEx.Word1 != Word1) {
        req.body.Word1 = Word1;
...
app.post("/studentAdminEmailID", async (req, res) => {
  try {
    ...
    const find2 = await StudentDetail.find({
      teachername_fk: req.session.emailOfStLo,
    }); //,{studentname:req.session.username}
    if (find2.length !== 0) {
      const find2 = await StudentDetail.findOne({
        teachername_fk: req.session.emailOfStLo,
        studentname: req.session.username,
        username: `${req.session.username}_${req.session.emailOfStLo}`,
      }); //,{studentname:req.session.username}
      if (!find2) {
...
app.get("/level", async (req, res) => {
  try {
    ...
    if (find == null) {
      ...
      const findsLevel1 = await ThreeWordS.find({
        exerciseName: req.session.levelName,
        wordLength: 3,
        email: req.session.emailOfStLo,
      });
      ...
      if (
        (find.level1 == "Not taken" || find.level1 == "Not completed") &&
        findsLevel1.length !== 0
      ) {
        req.session.leve1Completed = false;
        req.session.leve2Completed = false;
        req.session.leve3Completed = false;
      ...
      const findsLevel2 = await ThreeWordS.find({
        exerciseName: req.session.levelName,
        wordLength: 4,
        email: req.session.emailOfStLo,
      });
      ...
      if (
        (find.level2 == "Not taken" || find.level2 == "Not completed") &&
        findsLevel2.length !== 0
      ) {
        // req.session.leve1Completed = false
        req.session.leve2Completed = false;
        req.session.leve3Completed = false;
    ...
    req.session.save();
    const findsLevel = await ThreeWordS.find({
      exerciseName: req.session.levelName,
      email: req.session.emailOfStLo,
    });
    for (
      req.session.let = 0;
      req.session.let < findsLevel.length;
      req.session.let++
    ) {
      if (
        findsLevel[req.session.let].wordLength == 3 &&
        findsLevel[req.session.let].wordLength !== 4 &&
        findsLevel[req.session.let].wordLength !== 5
      ) {
    ...
    if (
      ...
      res.render("levelsSelecting.jade", {
        name: req.session.levelName,
        level1: req.session.leve1Completed,
        level2: req.session.leve2Completed,
        level3: req.session.leve3Completed,
...
app.get("/level1game", async (req, res) => {
  try {
    ...
    if (find.length == 0) {
      req.session.leve1Completed = true;
      req.session.save();
      return res.render("levelnotavai.jade", {
        level: "Level 1 is not available for this exercise",
        mode: req.session.mode,
      });
    }
    ...
    if (filter.length != 0) {
      ...
      req.session.save();
      res.render("level1.jade", {
        question: filter,
        level: req.session.start + 1,
        mode: req.session.mode,
        avail: req.session.onlyone,
      });
    } else {
      req.session.start = 0;
      req.session.leve1Completed = true;
      ...
      res.render("result.jade", {
        correct: req.session.correctAnswerLevel1,
        total: req.session.totalQuestionLevel1,
        mode: req.session.mode,
        avail: req.session.onlyone,
      });
    }
...
app.get("/level2game", async (req, res) => {
  try {
    ...
    if (find.length == 0) {
      ...
      req.session.save();
      return res.render("levelnotavai.jade", {
        level: "Level 2 is not available for this exercise",
        mode: req.session.mode,
      });
    }
    ...
    if (filter.length != 0) {
      ...
      res.render("result.jade", {
        correct: req.session.correctAnswerLevel2,
        total: req.session.totalQuestionLevel2,
        mode: req.session.mode,
        avail: req.session.onlyone,
      });
    }
...
app.get("/level3game", async (req, res) => {
  try {
    ...
    if (find.length == 0) {
      ...
      req.session.save();
      return res.render("levelnotavai.jade", {
        level: "Level 3 is not available for this exercise",
        mode: req.session.mode,
      });
    }
    ...
    if (filter.length != 0) {
      ...
      res.render("result.jade", {
        correct: req.session.correctAnswerLevel3,
        total: req.session.totalQuestionLevel3,
        mode: req.session.mode,
        avail: req.session.onlyone,
      });
    }
